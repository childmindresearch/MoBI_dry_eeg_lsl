...
void SendImpedanceData(DSI_Headset dsi_headset, lsl_outlet lsl_outlet);
float *sample;
...
int main( int argc, const char * argv[] )
{
  ...
  lsl_outlet outlet = InitLSL(h, streamName); CHECK; /* EEG stream outlet */
  
  printf("Initializing %s impedance outlet\n", streamName);
  lsl_outlet impedance_outlet = InitImpedanceLSL(h, streamName); CHECK; /* impedance stream outlet */
  ...
  while( KeepRunning==1 ){
    ...
    SendImpedanceData(h, impedance_outlet);
    }
  
  // Gracefully exit the program
  printf("\n%s will exit now...\n", argv[ 0 ]);
  lsl_destroy_outlet(outlet);
  lsl_destroy_outlet(impedance_outlet);
  return Finish( h );
}
...
int Finish( DSI_Headset h )
{
  ...
  // This sends a command to the headset to tell it to stop sending impedance driver.
